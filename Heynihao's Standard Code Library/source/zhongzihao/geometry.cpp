...
public:
	...
	P point(double angle){
		return P (o.x + std::cos(angle) * r, o.y + std::sin(angle) * r);
	}
};
...
std::pair <int, std::vector <P>> isCC(C c1, C c2){
	...
	if (sig(c1.r + c2.r - d) < 0) return {4, {}};
	if (sig(std::abs(c1.r - c2.r) - d) > 0) return {0, {}};
	double a = (c2.o - c1.o).angle();
	double p = (sqr(c1.r) + sqr(d) - sqr(c2.r)) / (2 * c1.r * d);
	...
	ret.push_back(p1);
	if (!sig(da)){
		return {!sig(c1.r + c2.r - d) ? 3 : 1, ret};
	}
	ret.push_back(p2);
	return {2, ret};
}
...
std::vector <L> tanCC(C c1, C c2){
	...
	double d = (c1.o - c2.o).abs(), dr = c1.r - c2.r, sr = c1.r + c2.r;
	if (sig(d - dr) < 0 || !sig(d) && !sig(dr)){
		return {};
	}
	double base = (c2.o - c1.o).angle();
	if (!sig(d - dr)){
		return {L (c1.point(base), c2.point(base))};
	}
	std::vector <L> ret;
	double ang = std::acos(dr / d);
	ret.push_back(L (c1.point(base + ang), c2.point(base + ang)));
	ret.push_back(L (c1.point(base - ang), c2.point(base - ang)));
	if (!sig(d - sr)){
		ret.push_back(L (c1.point(base), c2.point(base + PI)));
	}
	else if (sig(d - sr) > 0){
		ang = acos(sr / d);
		ret.push_back(L (c1.point(base + ang), c2.point(base + ang + PI)));
		ret.push_back(L (c1.point(base - ang), c2.point(base - ang + PI)));
	}
	return ret;
}
...
std::vector <P> convexHull(std::vector <P> &ps){ // 所有点共线时返回结果不定 
	const int N = 100010;
	static P stack[N];
...
double disConvexP(std::vector <P> &ps, P q){
	...
	auto in = [pin](P p1, P p2, P p3, P p4, P q){
		P o12 = (p1 - p2).rot(PI / 2);
		P o23 = (p2 - p3).rot(PI / 2);
		P o34 = (p3 - p4).rot(PI / 2);
